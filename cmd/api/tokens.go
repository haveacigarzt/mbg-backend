...
func (app *application) createAuthenticationTokenHandler(w http.ResponseWriter, r *http.Request) {
	fmt.Println("createAuthenticationTokenHandler called")
...
func (app *application) deleteAuthenticationTokenHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = app.writeJSON(w, http.StatusOK, envelope{"message": "authentication token successfully deleted"}, nil)
	if err != nil {
		app.serverErrorResponse(w, r, err)
	}
}

func (app *application) deleteCurrentAuthenticationTokenHandler(
	w http.ResponseWriter,
	r *http.Request,
) {
	token := app.contextGetToken(r)

	err := app.models.Tokens.DeleteByPlaintext(token)
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}
